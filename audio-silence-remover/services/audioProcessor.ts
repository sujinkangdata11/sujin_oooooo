...
export async function processAudio(file: File, silenceThresholdDb: number): Promise<AudioProcessingResult> {
        ...
        reader.onload = async (e) => {
            ...
            try {
                ...
                if (mergedSegments.length === 0) {
                   return reject(new Error("No audio detected above the silence threshold. Try a lower dB value."));
                }
                ...
                resolve({ originalUrl, processedUrl, processedBlob });

            } catch (err) {
                 reject(new Error("Could not decode audio file. It may be corrupted or in an unsupported format."));
            }
        };
        reader.onerror = () => {
